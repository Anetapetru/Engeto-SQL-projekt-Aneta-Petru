-- tabulka 1
CREATE OR REPLACE TABLE t_aneta_petru_project_SQL_primary_final AS
...
-- tabulka 2
CREATE OR REPLACE TABLE t_aneta_petru_project_SQL_secondary_final AS 
	SELECT 
		c.*,
		e.`year`,
		e.GDP,
...
FROM 
	(SELECT 
		...
		CASE 
			...
			ELSE 0
		END AS 'compare_wage'
	FROM t_aneta_petru_project_sql_primary_final spf 
	JOIN (
		SELECT 
			year_table AS year2,
			round(avg(value)) AS value,
			name_industry_branch
		FROM t_aneta_petru_project_sql_primary_final 
...
FROM t_aneta_petru_project_sql_primary_final pf 
WHERE (name = 'Mléko polotučné pasterované' OR name = 'Chléb konzumní kmínový') AND (year_table = (
	SELECT min(year_table) FROM t_aneta_petru_project_sql_primary_final pf ) OR year_table = (
	SELECT max(year_table) FROM t_aneta_petru_project_sql_primary_final pf ))
...
SELECT 
	...
	wage_tab.inc_wage
FROM t_aneta_petru_project_sql_secondary_final tsf 
JOIN (SELECT 
		country,
		`year`,
		GDP 
	FROM t_aneta_petru_project_sql_secondary_final
...
JOIN (
	...
	FROM t_aneta_petru_project_sql_primary_final tpf 
	JOIN (SELECT 
			year_table,
			name,
